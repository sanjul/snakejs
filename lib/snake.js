...
var Snake = function(canvas){
	...
	_this.eatSelf = function(){
		var headCoords = _bodySprites[0].getCoords();
		var bodyCoords;
		for (var i = 1; i < _length; i++){
			bodyCoords = _bodySprites[i].getCoords();
			if (headCoords.x == bodyCoords.x && headCoords.y == bodyCoords.y){
					return true;
			}
		}
		return false;
...
var Canvas = function(width, height){
	...
	_this.getRandomCoords = function(){
			var x = Math.round( Math.random() * (_this.getWidth() - 1));
			var y = Math.round( Math.random() * (_this.getHeight() - 1));
			return {"x" : x, "y" :y}
	}
...
var Painter = function(canvas, blockWiseWidth, blockWiseHeight){
	...
	var DEFAULT_COLOUR = '#81BEF7' ;//'#333333';
	...
	var drawCircle = function(x, y, radius){
		...
		_context.strokeStyle = DEFAULT_COLOUR;
		_context.fillStyle = DEFAULT_COLOUR;
		_context.stroke();
		_context.fill();
	}
